...
exports.crearBeneficios= catchAsync(async (req,res) =>{
...
exports.verBeneficio = catchAsync(async (req,res,next) =>{
    ...
    if(!Beneficio) return next(new AppError('No existe beneficio con este ID',404))
...
exports.editarBeneficio = catchAsync(async (req,res,next) =>{
   ...
   const beneficioEditado = await BeneficiosModel.findByIdAndUpdate(req.params.id,req.body,{
    new:true,
    runValidators:true
   })
    res.status(201).json({
...
exports.eliminarBeneficio = catchAsync(async (req, res, next) => {
    ...
    const beneficioEliminado = await BeneficiosModel.findByIdAndDelete(req.params.id)
