...
exports.crearDespido = catchAsync(async (req,res,next) =>{
    ...
    if(!req.body.tomoVacacionesb){
        sueldoDeVacaciones = calcular.sueldoVacaciones(usuario.sueldoFijo,req.body.diasDeVacaciones)
    }


    if(req.body.tipo === 'Desahucio'){
        const regalia = calcular.regalia(usuario.createdAt,usuario.sueldoFijo)
        prestaciones = calcular.calcularPrestaciones(usuario.createdAt,usuario.sueldoFijo,req.body.tomoVacaciones,sueldoDeVacaciones,regalia)
